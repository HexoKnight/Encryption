...
{
    /// <summary>
    /// again, not my code, I wasgoing to try to replicate the algorithm,
    /// but I just didn't understand what was going on so I just copied the code:
    /// https://gist.github.com/rharkanson/50fe61655e80488fcfec7d2ee8eff568
    /// but 'if (bs[bs.Length - 1] == 0) bs = bs.Take(bs.Length - 1).ToArray();' is my code
    /// </summary>
    class RandomBigInteger : Random
    {
        ...
        public BigInteger NextBigInteger(BigInteger start, BigInteger end)
        {
            ...
            if (bs[bs.Length - 1] == 0) bs = bs.Take(bs.Length - 1).ToArray();
